...
const changeYPosIfNecessary = (client) => () => {
  ...
  const newHeight = Math.min(
    client.frameGeometry.height,
    area.height - realNotchThickness,
  );
  client.frameGeometry = {
    x: client.frameGeometry.x,
    y: area.y + realNotchThickness,
...
const updateTaskbarThickness = (thickness) => {
  const thicknessExpression = thickness
    ? thickness.toString()
    : "notchThickness * scale";
  const plasmaScript = `
${getBasePlasmaScript()}
...
workspace.clientFullScreenSet.connect((client, fullscreen) => {
  ...
  if (fullscreen) {
    ...
    client.frameGeometry = {
      ...
      y: area.y + realNotchThickness,
      width: area.width,
      height: area.height - realNotchThickness,
    };
  }
